...
class Grafo:
    ...
    def dijkstra(self, inicio, fin):
        ...
        distancias[inicio] = 0
        prioridad = [(0, inicio)]  # (distancia, ciudad)
        caminos = {ciudad: None for ciudad in self.grafo}
        ...
        while prioridad:
            ...
            for ruta in self.grafo[ciudad_actual]:
                distancia_nueva = distancia_actual + ruta.distancia_km
                if distancia_nueva < distancias[ruta.destino]:
                    distancias[ruta.destino] = distancia_nueva
                    caminos[ruta.destino] = ciudad_actual
                    heapq.heappush(prioridad, (distancia_nueva, ruta.destino))
        ...
        print(f"Ruta más corta de {inicio} a {fin}: {' -> '.join(camino)} con distancia total de {distancias[fin]} km")
...
if _name_ == "_main_":
    ...
    print("\nBuscando la ruta más corta de Bucaramanga a Cali...")
    asistente.dijkstra("Bucaramanga", "Cali")
